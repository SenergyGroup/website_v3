...
function getFilteredLocations() {
  return allMarkers.filter(item => map.hasLayer(item.marker)).map(item => ({
    type: "Feature",
    properties: { type: item.marker.options.type },
    geometry: { type: "Point", coordinates: [item.marker.getLatLng().lng, item.marker.getLatLng().lat] }
  }));
}

function updatePoints() {
  var locationsToUse = document.getElementById('day').value ? getFilteredLocations() : foodLocations.map(function(location) {
    return {
      type: "Feature",
  ...
  points = L.geoJSON(locationsToUse);
}
...
// New code for points creation
var points = L.geoJSON(foodLocations.map(function(location) {
  return {
    type: "Feature",
    properties: { type: location.type },
    geometry: { type: "Point", coordinates: [location.lon, location.lat] }
...
function updatePolys (){
  console.log('updatePolys called');
  ...
  try {
    ...
    console.log('viewBounds:', viewBounds);
  	...
  	var pointsBounds = points.getBounds();
  	...
  	// Use the bounds to calculate the margins
    pointsN = pointsBounds.getNorth() + 1;  // Adjust as needed
    pointsE = pointsBounds.getEast() + 1;   // Adjust as needed
    pointsS = pointsBounds.getSouth() - 1;  // Adjust as needed
    pointsW = pointsBounds.getWest() - 1;   // Adjust as needed
      ...
      console.error('An error occurred in updatePolys:', error);
  }
}
